...
public class RocksDBStore implements MetadataStore {
    ...
    try {
      ...
      if (newDB) {
        ...
        List<byte[]> columnFamilies =
            RocksDB.listColumnFamilies(new Options(), dbLocation.getAbsolutePath());
        for (byte[] column : columnFamilies) {
          descriptors.add(new ColumnFamilyDescriptor(column, new ColumnFamilyOptions()));
        }
      }
      ...
      for (ColumnFamilyHandle handle : handles) {
        columnMap.put(new String(handle.getName()), handle);
      }
  ...
  private String getDefaultColumnFamily() {
    return new String(RocksDB.DEFAULT_COLUMN_FAMILY);
  }
  ...
  @Override
  public void dropCategory(String category) throws IOException {
    try {
      db.dropColumnFamily(getColumnFamilyHandle(category));
    } catch (RocksDBException e) {
      throw toIOException("Failed to dropColumnFamily", e);
    }
  }
}
